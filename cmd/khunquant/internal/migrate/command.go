...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to khunquant",
		Args:  cobra.NoArgs,
		Example: `  khunquant migrate
  khunquant migrate --from openclaw
  khunquant migrate --dry-run
  khunquant migrate --refresh
  khunquant migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.khunquant)")

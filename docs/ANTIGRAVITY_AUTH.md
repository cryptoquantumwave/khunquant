...
**Antigravity** (Google Cloud Code Assist) is a Google-backed AI model provider that offers access to models like Claude Opus 4.6 and Gemini through Google's Cloud infrastructure. This document provides a complete guide on how authentication works, how to fetch models, and how to implement a new provider in KhunQuant.
...
10. [Creating a New Provider in KhunQuant](#creating-a-new-provider-in-khunquant)
...
const antigravityPlugin = {
  ...
  register(api: KhunQuantPluginApi) {
...
type ProviderAuthResult = {
  ...
  configPatch?: Partial<KhunQuantConfig>;
  defaultModel?: string;
  notes?: string[];
};
```
...
- Go ≥ 1.21
- KhunQuant codebase (`pkg/providers/` and `pkg/auth/`)
...
Auth profiles are stored in `~/.khunquant/auth.json`:
...
## Creating a New Provider in KhunQuant

KhunQuant providers are implemented as Go packages under `pkg/providers/`. To add a new provider:
...
If your provider requires OAuth or special authentication, add a case to `cmd/khunquant/cmd_auth.go`:
...
khunquant auth login --provider your-provider

# List models (for Antigravity)
khunquant auth models

# Start the gateway
khunquant gateway

# Run an agent with a specific model
khunquant agent -m "Hello" --model your-model
```
...
export KHUNQUANT_AGENTS_DEFAULTS_MODEL=your-model

# Override provider settings
export KHUNQUANT_MODEL_LIST='[{"model_name":"your-model","model":"your-provider/model-name","api_key":"..."}]'
```
...
### "Token expired"
- Refresh OAuth tokens: `khunquant auth login --provider antigravity`
...
- Check auth profile storage: `~/.khunquant/auth.json`
- Re-run `khunquant auth login --provider antigravity`

...
const (
	...
	DescScanDeltaNeutralOpportunities = "Scan the top N crypto assets by market cap for delta-neutral funding-carry opportunities: batch-fetch funding rates, rank by annualized APR, optionally compute stability stats. Provider may be 'binance', 'okx', or empty/'all' to scan every supported exchange and combine the ranked results (each row tagged with its exchange). Returns a sorted table with funding metrics, direction labels, and a Spot column flagging whether each asset also has a spot pair on its exchange (assets with a perp but no spot are kept and marked NO-SPOT, not filtered out). Funding-only screen — validate with risk tools before trading."
)
